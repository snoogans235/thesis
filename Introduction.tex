...
When the molecular hydrogen has formed, it is still susceptable to photodissasociation from far ultra-violet photons.  The energy required in a single photon to break the bonds of molecular hydrogen is 14.5 eV which conviently is also enough energy to excite atomic hydrogen, so photodissasociation using a single photon is highly unlikely \citep{krumholz2014}.  However, a photon with an energy of 11-13.6 eV will not be able to excite atomic hydrogen, but will be able to excite the molecular hydrogen to its first and second excitation levels, the Lyman and Werner bands respectively.  The excited H$_2$ will eventually settle via photon emission to it's ground state with a finite probability of returning to two hydrogen atoms rather than maintaing it's molecular state \citep{krumholz2014}.

A dissassociation rate, $\zeta_{diss}$ can be obtained by scaling the excitation rate, $\zeta_{exc}$, by the fraction of excited hydrogen molecules that will settle to an atomic ground state, $\it{f}_{diss}$.  
