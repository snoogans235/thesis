...
\section{Making the Data Agree}

Initially the data do not agree on several levels.  The differences range from the filtering of the large scale structure from MAKEMAP, the range of resolutions, and differing beam shapes.   We have taken several steps to maximize the compatibility of the data some are simpler than other.  In order to account for the varying beam resolutions, we use a gaussian convolution to increase the resolution of our maps to the largest beam size in our dataset, 36$\arcsec$.  However, this only works when the beams are well approximated by a gaussian which is not the case for the 450$\mu$m beam.  The steps taken to match the resolution of the 450$\mu$m beam with the rest of the data set is given in $\S$ \ref{450_fix_sec}.  We can implement a feature in MAKEMAP that allows us to add fake sources into the data during production.  The fake source implementation allows us to remove the same amount of large scale structure from our ancillary data as was removed from the SCUBA-2 data.  The steps taken to prepare that ancillary data are described in $\S$ \ref{fakesource_sec}
...
\subsection{Accounting for the 450$\mu$ Error Beam}\label{450_fix_sec}

\begin{itemize}
  \item{we are not trying to reduce the 450 down to a single gaussian, instead we are trying to incorporate an error beam in the rest of the data}
  \item{incorporating the error beam involve taking advantage of the distributive nature of Fourier transforms, and will add a convolved error beam to the rest of our data}
  \item{give in equation form}
  \item{be sure to cite Gould belt consortium}
\end{itemize}
Taking the 450$\mu$m error beam into consideration is different than normal beam convolution in the sense that we are not changing just the lower resolution map--every map is being convolved.  We have to take these steps because convolving a double gaussian beam with a single gaussian kernel will not sufficiently remove the error component of our beam.\\

In order to accommodate the 450$\mu$m map's error beam, we used a method employed by another SCUBA-2 survey, the Gould-Belt Survey team.  This method used the distributive nature of the Fourier transform to create similar error components in the beam resolutions we were convolving to and from.  This relationship is shown in equation \ref{eq_GBSmethod} where $X_{desire}$ is the beam width of the resolution we are convolving to, $\alpha$ is the amplitude of the main beam of the 450$\mu$m emission, $\beta$ is the amplitude of the 450$\mu$m error beam, $X_{\alpha}$ and $X_{\beta}$ are the main and error beam of the 450$\mu$m observations, and $X_{450\mu m}$ is the composition of $X_{\alpha}$ and $X_{\beta}$ beams.
