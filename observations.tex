...
Because the the large scale removal of structure from MAKEMAP, the range of resolutions, and differing beam shapes, we took several steps we have taken several steps to maximize the compability of the data.  In order to account for the varying beam resolutions, we use a gaussain convolution //.  However, this only works when the beams are well approximated by a gaussian which is not the case for the 450$\mu$m beam.  The largest difference in Ability of Herschell to observe submm unimpeded by atmo and the configuration setup of THIGNs allows for a bit of extended structure to arise in NGC3627.

\subsection{Accounting for the 450$\mu$ Error Beam}

\subsection{Extended Structure Removal via MAKEMAP}
