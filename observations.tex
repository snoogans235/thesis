...
%  \tabletypesize{\footnotesize}
  ...
  \tablehead{\colhead{Observation} &
             \colhead{Beam Properties } &
             \colhead{RMS} &
             \colhead{Percentage of Emission Removed} }
  \startdata
    $\lambda$ & $\theta$ & \it{[Jy/pixel]}& \\ 
...
\begin{deluxetable}{cccccc}
  ...
  \tablehead{\colhead{Observation} &
             \multicolumn{3}{c}{Beam Properties} &
             \colhead{RMS} &
             \colhead{Percentage of Emission Removed} }
  \startdata
    Line & $\theta_{maj}$ & $\theta_{min}$ & $\theta_{PA}$ & \it{[$M_{\odot} / pc^2$]} \\
    HI & 10.6$\arcsec$ & 8.85$\arcsec$ & -49$^\circ$ & 0.760 & 39.6\% \\  
  \enddata
\end{deluxetable}
